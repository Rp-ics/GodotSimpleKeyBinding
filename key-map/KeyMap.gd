...
# ===== DEFAULT BINDINGS ===== #
# Each value is a dict { "type": "key"|"mouse"|"joy", "code": int } or null #
# This avoids numeric ambiguity: JOY_BUTTON_B=1 == MOUSE_BUTTON_LEFT=1 #
...
func _ready() -> void:
	if not grid:
		push_error("Node 'BindingsGrid' not found.")
		return
	if not reset_button:
		push_error("Node 'ResetButton' not found.")
		return
...
# ============================================================== #
# CREATE EVENT
# Accepts a dict { "type": "key"|"mouse"|"joy", "code": int }
# or null. The explicit type resolves numeric ambiguity
...
func _create_event(def) -> InputEvent:
	...
	match t:
		...
		"joy":
			...
			ev.button_index = c
			return ev
	push_warning("_create_event: type '%s' not recognized." % t)
	return null
	...
	for action in default_bindings:
		...
		for slot in SLOTS:
			...
			if err == OK:
				...
				if t == "none" or c == -1:
					e = null   # empty slot intentionally saved
				elif t == "key":
					var ev := InputEventKey.new()
					ev.keycode = c
					e = ev
